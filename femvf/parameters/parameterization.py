...
class FullParameterization(Parameterization):
    def __init__(self, model, ntime):
        ...
        times = vec.BlockVector((np.zeros(ntime),), labels=[('times',)])
        self._bvector = vec.concatenate_vec([ini_state, control, props, times]) #join_standard_to_bvector(ini_state, control, props, times)

    def convert(self):
        model = self.model
        subvec_bsizes = [bvec.size for bvec in (model.state0, model.control, model.props)] + [1]
...
class SubsetParameterization(Parameterization):
    ...
    def bvector(self):
        ...
        return vec.BlockVector(vecs, labels=[keys])
...
class NodalElasticModuli(Parameterization):
    ...
    def init_vector(self):
        vecs = [self.model.props['emod'].copy()]
        labels = ['emod']
...
class ElasticModuliAndInitialState(Parameterization):
    ...
    """
    def init_vector(self):
        vecs = [self.model.state0.copy(), self.model.props['emod'].copy()]
