...
from femvf.vis.xdmfutils import (
    export_mesh_values,
    write_xdmf,
    XDMFArray,
    make_format_dataset,
...
@pytest.fixture(params=['M5_BC--GA3--DZ0.00', 'M5_BC--GA3--DZ1.50'])
...
def model(mesh_path):
    ...
    model = load_transient_fsi_model(
        mesh_path, None, SolidType=Rayleigh, FluidType=BernoulliAreaRatioSep, zs=zs
    )
...
def state_fpath(model, state_controls_prop, mesh_name):
    ...
    with sf.StateFile(model, state_fpath, mode='w') as f:
        ...
        integrate(model, f, state, controls, prop, times=np.linspace(0, 1e-2, 10))
